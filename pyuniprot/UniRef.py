...
class UniRef:
    ...
    def __init__(
        ...
        self._uniref_json_url = (
            f"https://rest.uniprot.org/uniref/UniRef{self.thresh}_{self.uniprot_id}.json"
        )
        self._uniref_json_file: str | os.PathLike | io.StringIO | None = None
        json_file = Path(self._local_download_dir, f"UniRef{self.thresh}_{self.uniprot_id}.json")
        if json_file.exists():
    ...
    def _get_uniref_json(self):
        ...
        if self.uniref_json_file is None:
            try:
                ...
                if self.save_json:
                    json_file = Path(self.local_download_dir, f"UniRef{self.thresh}_{self.uniprot_id}.json")
                    with open(json_file, "w", encoding="utf-8") as j_file:
                        j_file.write(json_content)

                self.uniprot_txt_file = json_file
